...
class Permission:
    ...
    def get_permission(self):
        query = "SELECT name, permission_id FROM permissions WHERE name = '{}'".format(self._name)
        result = self.db_manager.do_select(query)
        if result:
            self._name, self._permission_id = result[0]
        return result
